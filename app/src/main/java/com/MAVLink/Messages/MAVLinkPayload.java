...
public class MAVLinkPayload {
    ...
    public static final int MAX_PAYLOAD_SIZE = 0xFF;
...
//        result |= ((payload.get(index + 0) & 0xFFFFL));
        result |= ((payload.get(index + 3) & 0xFFFFL) << 24) & 0xff000000;
        result |= ((payload.get(index + 2) & 0xFFFFL) << 16) & 0x00ff0000;
        result |= ((payload.get(index + 1) & 0xFFFFL) << 8) & 0x0000ff00;
        result |= ((payload.get(index + 0) & 0xFFFFL)) & 0x000000ff;
        index += 4;
        return result;
    }
    ...
    public long getLong() {
        long result = 0;
        result |= (payload.get(index + 7) & 0xFFFFL) << 56& 0xff00000000000000L;
        result |= (payload.get(index + 6) & 0xFFFFL) << 48& 0x00ff000000000000L;
        result |= (payload.get(index + 5) & 0xFFFFL) << 40& 0x0000ff0000000000L;
        result |= (payload.get(index + 4) & 0xFFFFL) << 32& 0x000000ff00000000L;
        result |= (payload.get(index + 3) & 0xFFFFL) << 24& 0x00000000ff000000L;
        result |= (payload.get(index + 2) & 0xFFFFL) << 16& 0x0000000000ff0000L;
        result |= (payload.get(index + 1) & 0xFFFFL) <<  8& 0x000000000000ff00L;
        result |= (payload.get(index + 0) & 0xFFFFL)&       0x00000000000000ffL;
